...
author: "Oliver Atwood, Yingtong Zhong, adapted from Ken Steif, 2019. Eds. Michael Fichman & Jenna Epstein"
date: "05/02/2023"
output:
  html_document:
    toc: true
...
## 2.4b New Highway
```{r read and plot highway 2, warning = FALSE, message= FALSE,results = "hide"}
CvilleHighways2 <-
st_read("https://raw.githubusercontent.com/Yingtong-Z/sprawl-forecasting/992db0a2e854c764df2bbafcaac6d424975d4bdc/data/Cville_Highways2.geojson") %>%
  st_transform(st_crs(CvilleMSA)) %>%
  st_intersection(CvilleMSA)
...
highwayPoints_fishnet2 <- 
  aggregate(highwayPoints2, CvilleMSA, mean) %>%
...
dat <- 
  cbind(
    fishnet, highwayPoints_fishnet, highwayPoints_fishnet2, fishnetGraduate00,
  ...
  dplyr::select(lc_change, developed, forest, farm, wetlands, otherUndeveloped, slope,water,
                ...
                total_vacant_households,total_white,total_population2020,pop_Change_00_20,distance_highways, distance_highways2, lagDevelopment, total_housing_units2020, total_white2020) %>%
  st_join(studyAreaCounties) %>%
  mutate(NAME = ifelse(NAME == "Charlottesville", "Charlottesville", "Albemarle")) %>% 
  mutate(developed19forlag = ifelse(lc_change == 1 & developed == 1, 0, developed)) %>%
  filter(water == 0) 
...
dat <-
  dat %>%
  mutate(lagDevelopment = nn_function(xyC(.), xyC(filter(.,developed19forlag == 2)),2))
```

...
CvilleMSA <-
  ...
  st_read("/Users/oliveratwood/Documents/GitHub/sprawl-forecasting/data/Cville_Tesselated_Coarser.geojson") %>%
...
CvilleHighways <-
  ...
  # st_read("https://raw.githubusercontent.com/Yingtong-Z/sprawl-forecasting/b528d725eaea5dd778c41fa644b5e6f71d6291b2/data/Cville_Highways2.geojson") %>%
  st_transform(st_crs(CvilleMSA)) %>%
...
```{r}
# SlopeRaster <- raster("/Users/oliveratwood/Documents/GitHub/sprawl-forecasting/data/Cville_Slope.tif")
# 
# # Use aggregateRaster to aggregate the Slope raster with dat
# aggregated_slope <- aggregateRaster(SlopeRaster, dat)
# 
# # Convert the aggregated Slope raster to a data frame
# aggregated_slope_df <- as.data.frame(aggregated_slope, xy = TRUE)
# 
# # Combine the aggregated Slope raster data frame with the dat dataset
# dat <- aggregated_slope_df %>%
#   st_as_sf(coords = c("x", "y"), crs = st_crs(dat)) %>%
#   st_join(dat) %>%
#   st_cast("POLYGON")
```
...
```
Reformat 2019 Land Cover for Model, preserving 2001 Land Cover values as new columns
```{r}
dat2 <- dat2 %>%
    mutate(farm01 = farm) %>%

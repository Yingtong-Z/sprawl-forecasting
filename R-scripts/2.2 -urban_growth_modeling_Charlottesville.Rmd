...
CvilleMSA <-
  ...
  st_read("/Users/oliveratwood/Documents/GitHub/sprawl-forecasting/data/Cville_Tesselated_Coarse.geojson") %>%
...
dat <- 
  ...
  dplyr::select(lc_change, developed, forest, farm, wetlands, otherUndeveloped, water,
                ...
                total_vacant_households,total_white,total_population2020,pop_Change_00_20,distance_highways, lagDevelopment) %>%
...
Next, a table of land cover conversion between 2001 and 2019 is created. The table suggests for instance, that 0.71% of farmland regionally was converted to development between 2001 and 2019. This indicator should be interpreted in the context of the scale changes we imposed on the data by moving from a 30ft by 30ft raster to a 4000ft by 4000ft fishnet. This is the same reason why the table suggests `developed` area was then “developed”.
...
Model4 <- glm(lc_change ~ wetlands + forest  + farm + otherUndeveloped + lagDevelopment + total_population +
                total_population2020 + distance_highways,
                family="binomial"(link="logit"), data = datTrain)  

Model5 <- glm(lc_change ~ wetlands + forest  + farm + otherUndeveloped + lagDevelopment + total_population +
                total_population2020 + pop_Change_00_20 + distance_highways,
                family="binomial"(link="logit"), data = datTrain)               

Model6 <- glm(lc_change ~ wetlands + forest  + farm + otherUndeveloped + lagDevelopment + total_population +
                total_population2020 + pop_Change_00_20 + distance_highways +
                total_graduate_degree_holders + total_housing_units + total_vacant_households + total_white,
                family="binomial"(link="logit"), data = datTrain) 
...
# reassign cells with more than one lc value to just one based on hierarchy shown here
dat2 <- dat2 %>%
    mutate(farm = ifelse(developed19 == 0 & farm19 == 1, 1, 0)) %>%
    mutate(forest = ifelse(developed19 == 0 & farm19 == 0 & forest19 == 1, 1, 0)) %>%
    mutate(wetlands = ifelse(developed19 == 0 & farm19 == 0 & forest19 == 0 & wetlands19 == 1, 1, 0)) %>%
    mutate(water = ifelse(developed19 == 0 & farm19 == 0 & forest19 == 0 & wetlands19 == 0 & water19 == 1, 1, 0)) %>%
    mutate(otherUndeveloped = ifelse(developed19 == 0 & farm19 == 0 & forest19 == 0 & wetlands19 == 0 & water19 == 0 & otherUndeveloped19 == 1, 1, 0))

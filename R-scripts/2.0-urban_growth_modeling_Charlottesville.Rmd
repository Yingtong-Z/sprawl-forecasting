...
CvilleMSA <- 
  st_read("/Users/angel_zhong/Documents/GitHub/sprawl-forecasting/data/Cville_Hex.geojson") %>%
  st_transform('EPSG:2284')

lc_change = raster("/Users/angel_zhong/Documents/GitHub/sprawl-forecasting/data/LC_Change_2001_2019.tif")
```
...
```{r, warning = FALSE, message = FALSE}
lc_2001 <- raster("/Users/angel_zhong/Documents/GitHub/sprawl-forecasting/data/Cville_LC_2001.tif")
...
fishnetHHunit00 <-
  ...
  left_join(CvilleMSA %>%
              mutate(fishnetID = as.character(fishnetID)),
            ., by=c("fishnetID"='fishnetID')) %>% 
  mutate(total_housing_units = replace_na(total_housing_units,0)) %>%
  dplyr::select(total_housing_units)
...
FishnetCensusData <- FishnetCensusData %>%
  mutate(pop_2000 = replace_na(list(fishnetPopulation00), 0)) %>%
  mutate(white_2000 = replace_na(list(total_white), 0))
...
FishnetCensusData <- FishnetCensusData %>%
            dplyr::select(fishnetID, total_population, total_white)
...
CvilleHighways <-
  st_read("/Users/angel_zhong/Documents/GitHub/sprawl-forecasting/data/Cville_Highways.geojson") %>%
  st_transform(st_crs(CvilleMSA)) %>%
  st_intersection(CvilleMSA)

CvilleRoads <-
  st_read("/Users/angel_zhong/Documents/GitHub/sprawl-forecasting/data/Cville_Roads.geojson") %>%
  st_transform(st_crs(CvilleMSA)) %>%
  st_intersection(CvilleMSA)
```
...
dat <- 
  cbind(
    fishnet, highwayPoints_fishnet, fishnetPopulation00, aggregatedRasters) %>%
  dplyr::select(lc_change, developed, forest, farm, wetlands, otherUndeveloped, water,
                total_population, distance_highways, lagDevelopment) %>%
...
```{r, warning = FALSE, message = FALSE}
modelList <- paste0("Model", 1:3)
map_dfc(modelList, function(x)pR2(get(x)))[4,] %>%
  setNames(paste0("Model",1:3)) %>%

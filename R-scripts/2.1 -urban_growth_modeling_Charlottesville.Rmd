...
# forest_data <- rast(lc_2001) %>% 
#   na.omit %>% 
#   filter(value %in% c(41, 42, 43)) %>%
...
#     aes(x, y, fill = forest)
#   ) +
#   scale_fill_manual(values = c("forest" = "green"), name = "") +
#   labs(title = "Land Cover, 2001") +
#   mapTheme +
#   theme(legend.direction = "horizontal")
```
...
ggplot()+
  geom_sf(data = fishnetPopulation00, 
          aes(fill = total_population), color = "transparent")
```
...
```{r read and plot highway, warning = FALSE, message= FALSE}
CvilleHighways <-
...
ggplot() +
  ...
  mapTheme
```
```{r read and plot roads, warning = FALSE, message= FALSE}
# CvilleRoads <-
#   st_read("https://raw.githubusercontent.com/Yingtong-Z/sprawl-forecasting/b528d725eaea5dd778c41fa644b5e6f71d6291b2/data/Cville_Roads.geojson") %>%
#   st_transform(st_crs(CvilleMSA)) %>%
#   st_intersection(CvilleMSA)
# 
# ggplot() +
...
dat <- 
  cbind(
    fishnet, highwayPoints_fishnet,fishnetGraduate00,fishnetHHunit00,fishnetPopulation00,fishnetVacHH00,fishnetWhiteP00,fishnetPopulation20,aggregatedRasters) %>%
  dplyr::select(lc_change, developed, forest, farm, wetlands, otherUndeveloped, water,
                total_population,total_graduate_degree_holders,total_housing_units,total_vacant_households,total_white,total_population2020,distance_highways, lagDevelopment) %>%
...
Model3 <- glm(lc_change ~ wetlands + forest  + farm + otherUndeveloped + lagDevelopment + total_population + distance_highways, 
              family="binomial"(link="logit"), data = datTrain)          
#!!!!!!!!!!!!!!!!add slope variable?

Model4 <- glm(lc_change ~ wetlands + forest  + farm + otherUndeveloped + lagDevelopment + total_population + total_graduate_degree_holders + total_housing_units + total_vacant_households + total_white + distance_highways, 
              family="binomial"(link="logit"), data = datTrain)   
...
```{r, warning = FALSE, message = FALSE}
modelList <- paste0("Model", 1:4)
map_dfc(modelList, function(x)pR2(get(x)))[4,] %>%
  setNames(paste0("Model",1:4)) %>%
...
testSetProbs <- 
  data.frame(class = datTest$lc_change,
             probs = predict(Model3, datTest, type="response")) 
...
predsForMap <-         
  dat %>%
    mutate(probs = predict(Model3, dat, type="response") ,
           ...
           Threshold_15_Pct =  as.factor(ifelse(probs >= 0.15 ,1,0))) %>%
    dplyr::select(lc_change,Threshold_5_Pct,Threshold_15_Pct) %>%
...
ConfusionMatrix.metrics <-
  dat %>%
    ...
    mutate(TrueP_05 = ifelse(lc_change  == 1 & Threshold_5_Pct == 1, 1,0),
           ...
           TrueP_15 = ifelse(lc_change  == 1 & Threshold_15_Pct == 1, 1,0),
           TrueN_15 = ifelse(lc_change  == 0 & Threshold_15_Pct == 0, 1,0)) %>%
...
At this point, a simple but useful model has been trained to predict urban development between 2001 and 2019 as a function of baseline features from 2001 including land cover, built environment and population. Next, we are going to update our features to reflect a 2010 baseline. Having done so, predictions from our new model would then be for 2040.
...
2040 population projections: Albemarle 138523, Charlottesville 48939
```{r, warning = FALSE, message = FALSE}
...
dat_infill %>%
  ggplot() +  
  geom_sf(aes(fill = factor(ntile(predict_2040.infill, 5))), color = "transparent") +
  scale_fill_manual(values = palette5,
                    labels = substr(quintileBreaks(dat_infill, "predict_2040.infill"), 1, 4),
                    name = "Quintile\nBreaks") +
...
ggplot() +
  geom_point(data=dat2, aes(x=xyC(dat2)[,1], y=xyC(dat2)[,2], colour=as.factor(sensitive_lost19), size = 0.1)) +
  scale_colour_manual(values = palette2,
...
county_specific_metrics <- 
  ...
  mutate(Development_Demand = predict(Model3, dat2, type="response")) %>%

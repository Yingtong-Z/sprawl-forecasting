...
(maybe add a fixed effect variable of municipality)
(maybe add a fixed effect variable of existing land use)
...
2.4: Population data is downloaded from the U.S. Census and joined to the fishnet by distributing Census Tract population totals proportionally to each grid cell.

2.5: Highway vectors are downloaded from Texas State open data website and used to wrangle highway proximity features.
...
!!!Note: Spatial Reference for Charlottesville is EPSG:2284: NAD83 / Virginia South (ftUS)

```{r load_data, warning = FALSE, message = FALSE, results = "hide"}
CvilleMSA <- 
  st_read("/Users/oliveratwood/Documents/GitHub/sprawl-forecasting/data/Cville_Hex.geojson") %>%
  st_transform('EPSG:2284')

lc_change = raster("/Users/oliveratwood/Documents/GitHub/sprawl-forecasting/data/LC_Change_2001_2019.tif")
```
...
Convert raster to 'sf' point layer, join points to fishnet, and plot
```{r, warning = FALSE, message = FALSE}
# The code below converts the raster to an `sf` point layer and then joins the points to the fishnet with `aggregate`. 
# This works well because the raster and the fishnet are of the same spatial resolution.

# The raster is converted to points, which makes its joining to the vector fishnet a bit faster.
# convert the raster to an `sf` point layer
changePoints <-
  rasterToPoints(lc_change2) %>%
  as.data.frame() %>%
  st_as_sf(coords = c("x", "y"), crs = st_crs(CvilleMSA))

# join the points to the fishnet with `aggregate`.
# Finally, the fishnet variable `lc_change` is created that is `1` where new development has occurred and `0` where it has not.
# This is our dependent variable and encoded as a factor.
fishnet <- 
...
ggplot() +
  ...
  geom_point(data=fishnet, 
             aes(x=xyC(fishnet)$x, y=xyC(fishnet)$y, colour=lc_change), size = .1) +
...
```{r, warning = FALSE, message = FALSE}
lc_2001 <- raster("/Users/oliveratwood/Documents/GitHub/sprawl-forecasting/data/Cville_LC_2001.tif")
...
```{r, warning = FALSE, message = FALSE}
theRasterList <- c(developed,farm,forest,wetlands,water,otherUndeveloped)

aggregatedRasters <-
  aggregateRaster(theRasterList, CvilleMSA) %>%
  dplyr::select(developed,farm,forest,wetlands,water,otherUndeveloped) %>%
...
cvam_2000_census <- cvam_2000_census %>% 
  st_transform(st_crs(CvilleMSA))
```
!!!!!!!!interpolate census data
```{r}
# Interpolate total_population
fishnetPopulation00 <-
  st_interpolate_aw(cvam_2000_census["total_population"], CvilleMSA, extensive=TRUE) %>%
  as.data.frame(.) %>%
  rownames_to_column(var = "fishnetID")

# Interpolate total_white
fishnetWhite00 <-
  st_interpolate_aw(cvam_2000_census["total_white"], CvilleMSA, extensive=TRUE) %>%
  as.data.frame(.) %>%
  rownames_to_column(var = "fishnetID")

# Interpolate total_housing_units
fishnettotal_housing_units <-
  st_interpolate_aw(cvam_2000_census["total_housing_units"], CvilleMSA, extensive=TRUE) %>%
  as.data.frame(.) %>%
  rownames_to_column(var = "fishnetID")

# Interpolate total_vacant_households
fishnettotal_vacant_households <-
  st_interpolate_aw(cvam_2000_census["total_vacant_households"], CvilleMSA, extensive=TRUE) %>%
  as.data.frame(.) %>%
  rownames_to_column(var = "fishnetID")

# Interpolate median_household_income
fishnetmedian_household_income <-
  st_interpolate_aw(cvam_2000_census["median_household_income"], CvilleMSA, extensive=TRUE) %>%
  as.data.frame(.) %>%
  rownames_to_column(var = "fishnetID")

# Interpolate total_graduate_degree_holders
fishnettotal_graduate_degree_holders <-
  st_interpolate_aw(cvam_2000_census["total_graduate_degree_holders"], CvilleMSA, extensive=TRUE) %>%
  as.data.frame(.) %>%
  rownames_to_column(var = "fishnetID")
...
# Population
FishnetCensusData <- fishnetPopulation00

# White
FishnetCensusData <- fishnetWhite00 %>%
  left_join(FishnetCensusData %>%
            mutate(fishnetID = as.character(fishnetID)),
            by = c("fishnetID" = 'fishnetID'))

# total_housing_units
FishnetCensusData <- fishnettotal_housing_units %>%
  left_join(FishnetCensusData %>%
            mutate(fishnetID = as.character(fishnetID)),
            by = c("fishnetID" = 'fishnetID'))
...
# total_graduate_degree_holders


# Replace NA values with 0s
FishnetCensusData <- FishnetCensusData %>%
            mutate(pop_2000 = replace_na(fishnetPopulation00, 0)) %>%
            mutate(white_2000 = replace_na(total_white, 0))

# Calculate population change
FishnetCensusData <- FishnetCensusData %>% 
  mutate(pop_Change = pop_2020 - pop_2000)

# Overwrite dataset with only the cleaned, correctly named rows
FishnetCensusData <- FishnetCensusData %>%
            dplyr::select(fishnetID, pop_2000, white_2000)
...
CvilleHighways <-
  st_read("/Users/oliveratwood/Documents/GitHub/sprawl-forecasting/data/Cville_Highways.geojson") %>%
  st_transform(st_crs(CvilleMSA)) %>%
  st_intersection(CvilleMSA)

CvilleRoads <-
  st_read("/Users/oliveratwood/Documents/GitHub/sprawl-forecasting/data/Cville_Roads.geojson") %>%
  st_transform(st_crs(CvilleMSA)) %>%
  st_intersection(CvilleMSA)
```
...
ggplot() +
  ...
  mapTheme
```
```{r plot_roads, warning = FALSE, message= FALSE}
ggplot() +
  geom_point(data=fishnet, 
             aes(x=xyC(fishnet)[,1], y=xyC(fishnet)[,2],colour=lc_change),size=0.1) +
  geom_sf(data=CvilleRoads, size = 0.1) +
  scale_colour_manual(values = palette2,
                      labels=c("No Change","New Development")) +
  labs(title = "New Development and Highways",
       subtitle = "As fishnet centroids") +
...
road_raster <- 
  as(CvilleRoads,'Spatial') %>%
  rasterize(.,emptyRaster)

road_raster_distance <- distance(road_raster)
names(road_raster_distance) <- "distance_roads"

RoadPoints <-
  rasterToPoints(road_raster_distance) %>%
  as.data.frame() %>%
  st_as_sf(coords = c("x", "y"), crs = st_crs(CvilleMSA))

RoadPoints_fishnet <- 
  aggregate(RoadPoints, CvilleMSA, mean) %>%
  mutate(distance_roads = ifelse(is.na(distance_roads),0,distance_roads))

ggplot() +
  geom_sf(data=CvilleMSA) +
  geom_point(data=RoadPoints_fishnet, aes(x=xyC(RoadPoints_fishnet)[,1], 
                                             y=xyC(RoadPoints_fishnet)[,2], 
                 colour=factor(ntile(distance_roads,5))),size=1.5) +
  scale_colour_manual(values = palette5,
                      labels=substr(quintileBreaks(RoadPoints_fishnet,"distance_highways"),1,8),
                      name="Quintile\nBreaks") +
  geom_sf(data=CvilleRoads, colour = "red") +
  labs(title = "Distance to Roads",
       subtitle = "As fishnet centroids; Roads visualized in red") +
  mapTheme
```
...
studyAreaCounties <- 
  counties("Virginia") %>%
  st_transform(st_crs(CvilleMSA)) %>%
  dplyr::select(Albemarle, Charlottesville)
```
...
dat <- 
  cbind(
    fishnet, highwayPoints_fishnet, FishnetCensusData, aggregatedRasters) %>%
  dplyr::select(lc_change, developed, forest, farm, wetlands, otherUndeveloped, water,
                pop_2000, pop_2010, pop_Change, distance_highways, distance_roads, lagDevelopment) %>%
...
countyPopulation_2040 %>%
  ...
  scale_fill_manual(values = palette2,
                    labels=c("2019","2001"),
                    name="Population") +
  labs(title="Population Change by County: 2001 - 2019",
...
ggplot() +
  ...
  geom_point(data=fortBend_landUse, aes(x=xyC(fortBend_landUse)[,1], 
                                        ...
                                        shape = 15, size = 2) +
  geom_sf(data=st_intersection(CvilleHighways,filter(studyAreaCounties, NAME=="Fort Bend")), size=2) +
